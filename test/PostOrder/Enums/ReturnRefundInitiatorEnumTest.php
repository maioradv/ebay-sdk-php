<?php
/**
 * DO NOT EDIT THIS FILE!
 *
 * This file was automatically generated from external sources.
 *
 * Any manual change here will be lost the next time the SDK
 * is updated. You've been warned!
 */

namespace maiorADV\eBaySDK\Test\PostOrder\Enums;

use maiorADV\eBaySDK\PostOrder\Enums\ReturnRefundInitiatorEnum;

class ReturnRefundInitiatorEnumTest extends \PHPUnit_Framework_TestCase
{
    private $obj;

    protected function setUp()
    {
        $this->obj = new ReturnRefundInitiatorEnum();
    }

    public function testCanBeCreated()
    {
        $this->assertInstanceOf('\maiorADV\eBaySDK\PostOrder\Enums\ReturnRefundInitiatorEnum', $this->obj);
    }
}
