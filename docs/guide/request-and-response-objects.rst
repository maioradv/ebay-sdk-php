...
.. code-block:: php
    ...
    use \maiorADV\eBaySDK\Finding\Types;
...
Properties of the objects are named after the fields found in the offical eBay documentation. The SDK will throw a ``maiorADV\eBaySDK\Exceptions\UnknownPropertyException`` if you get or set a property that does not exist.
...
A ``maiorADV\eBaySDK\Exceptions\InvalidPropertyTypeException`` is thrown if a value of the wrong type is assigned to a property.
...
You can disable this type checking by assigning ``false`` to the static property ``\maiorADV\eBaySDK\Sdk::$STRICT_PROPERTY_TYPES``.

.. code-block:: php

  \maiorADV\eBaySDK\Sdk::$STRICT_PROPERTY_TYPES = false;
